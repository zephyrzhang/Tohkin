...
function gad_admin_handle_other_options($info_message = '')
{
  if( isset($_POST['SubmitOptions']) ) 
  {
    ...
    if( isset($_POST['ga_forget_all']) )
    {
      ...
      gad_admin_plugin_options('Everything Reset');
      return;
    }
  ...
  $ga = new GALib(get_option('gad_auth_token'), '', get_option('gad_cache_timeout') !== false ? get_option('gad_cache_timeout') : 60);
...
function gad_posts_pages_custom_column($column_name, $post_id, $post_or_page)
{
  ...
  {
    $ga = new GALib(get_option('gad_auth_token'), get_option('gad_account_id'), get_option('gad_cache_timeout') !== false ? get_option('gad_cache_timeout') : 60);
...
function dashboard_gad()
{
  ...
  {
    $goal_data_tmp = $ga->goals_for_date_period($start_date, $end_date, array(get_option('gad_goal_one') !== false, get_option('gad_goal_two') !== false, get_option('gad_goal_three') !== false, get_option('gad_goal_four') !== false));
    $error_type = gad_request_error_type($ga);
    if($error_type == 'perm') return;
    else if($error_type == 'retry') $goal_data_tmp = $ga->goals_for_date_period($start_date, $end_date, array(get_option('gad_goal_one') !== false, get_option('gad_goal_two') !== false, get_option('gad_goal_three') !== false, get_option('gad_goal_four') !== false));
...
{
  echo '<tr><td>' . get_option('gad_goal_one') . '</td><td width="20px">&nbsp;</td><td>' . $goal_data['ga:goal1Completions'] . ' (' . round($goal_data['ga:goal1Completions'] / $summary_data['value']['ga:visits'] * 100, 2) . '%)</td></tr>';
}
if( get_option('gad_goal_two') !== false )
{
  echo '<tr><td>' . get_option('gad_goal_two') . '</td><td width="20px">&nbsp;</td><td>' . $goal_data['ga:goal2Completions'] . ' (' . round($goal_data['ga:goal2Completions'] / $summary_data['value']['ga:visits'] * 100, 2) . '%)</td></tr>';
}
?>
        </table>
      </div>
...
{
  echo '<tr><td>' . get_option('gad_goal_three') . '</td><td width="20px">&nbsp;</td><td>' . $goal_data['ga:goal3Completions'] . ' (' .  round($goal_data['ga:goal3Completions'] / $summary_data['value']['ga:visits'] * 100, 2) . '%)</td></tr>';
}
if( get_option('gad_goal_four') !== false ) 
{
  echo '<tr><td>' . get_option('gad_goal_four') . '</td><td width="20px">&nbsp;</td><td>' . $goal_data['ga:goal4Completions'] . ' (' .  round($goal_data['ga:goal4Completions'] / $summary_data['value']['ga:visits'] * 100, 2) . '%)</td></tr>';
}
?>
        </table>
      </div>
